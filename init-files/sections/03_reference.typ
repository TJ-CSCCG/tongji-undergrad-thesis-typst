...
在学术论文或科技报告中，通常需要引用相关文献以支持观点或论证。为了方便读者查阅，我们需要在论文中标注参考文献。在 Typst  中，可使用 ```typ @``` 命令引用参考文献。参考文献的外观应符合国标 GB/T 7714-2015 的要求。请注意，Typst对国标的支持仍处于实验阶段，存在不兼容的情况（如专利引用格式），请谨慎使用。
...
在文档中，交叉引用是指引用文档中的某个标签或标记，例如章节、图表、公式或页码等。在 Typst 中，可以使用 ```typ <label>``` 命令为文档中的对象添加标签，使用 ```typ @``` 命令进行引用。
...
由于我们对 Typst 中的浮动体（图表等）和公式的计数器进行了特殊处理，因此在引用浮动体和公式时，需要一些额外的操作。请参见@floats。